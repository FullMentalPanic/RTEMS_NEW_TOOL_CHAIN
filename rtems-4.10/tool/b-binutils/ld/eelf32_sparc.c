...
static lang_output_section_statement_type *gldelf32_sparc_place_orphan
  (asection *, const char *, int);

static void
gldelf32_sparc_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
static bfd_boolean
gldelf32_sparc_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldelf32_sparc_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldelf32_sparc_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldelf32_sparc_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needed != NULL)
	{
	  ...
	  lang_for_each_input_file (gldelf32_sparc_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldelf32_sparc_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldelf32_sparc_search_needed (const char *path,
				    struct dt_needed *n, int force)
{
  ...
  if (name[0] == '/')
    return gldelf32_sparc_try_needed (n, force);
  ...
  while (1)
      ...
      else
	{
      ...
      if (gldelf32_sparc_try_needed (&needed, force))
	return TRUE;
...
static void
gldelf32_sparc_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldelf32_sparc_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf32_sparc_write_build_id_section (bfd *abfd)
{
...
gldelf32_sparc_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf32_sparc_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf32_sparc_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldelf32_sparc_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldelf32_sparc_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldelf32_sparc_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldelf32_sparc_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldelf32_sparc_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldelf32_sparc_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldelf32_sparc_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldelf32_sparc_find_exp_assignment (exp->binary.lhs);
      gldelf32_sparc_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldelf32_sparc_find_exp_assignment (exp->trinary.cond);
      gldelf32_sparc_find_exp_assignment (exp->trinary.lhs);
      gldelf32_sparc_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldelf32_sparc_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldelf32_sparc_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldelf32_sparc_find_exp_assignment (s->assignment_statement.exp);
}
...
gldelf32_sparc_before_allocation (void)
{
  ...
  lang_for_each_statement (gldelf32_sparc_find_statement_assignment);
...
static bfd_boolean
gldelf32_sparc_open_dynamic_archive
...
static lang_output_section_statement_type *
gldelf32_sparc_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldelf32_sparc_after_allocation (void)
{
  bfd_boolean need_layout = bfd_elf_discard_info (link_info.output_bfd,
						  &link_info);
  gldelf32_sparc_map_segments (need_layout);
}
...
static void
gldelf32_sparc_list_options (FILE * file)
{
...
struct ld_emulation_xfer_struct ld_elf32_sparc_emulation =
{
  ...
  NULL,
  gldelf32_sparc_open_dynamic_archive,
  gldelf32_sparc_place_orphan,
  NULL,
  NULL,
  gldelf32_sparc_add_options,
  gldelf32_sparc_handle_option,
  NULL,
  gldelf32_sparc_list_options,
  gldelf32_sparc_load_symbols,
  NULL,
  NULL
};
